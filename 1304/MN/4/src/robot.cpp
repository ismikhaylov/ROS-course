...
public:
    ...
    NavigationManager(const Parameters& parameters, ros::NodeHandle& nodeHandle):
            m_targetPosition(parameters.getTargetPosition()) {
        m_odometrySubscriber = nodeHandle.subscribe<nav_msgs::Odometry>("/odom", 100, &NavigationManager::updatePosition, this);
        m_laserScanSubsriber = nodeHandle.subscribe<sensor_msgs::LaserScan>("/base_scan", 100, &NavigationManager::anaizeLaserScans, this);
...
private:
    ...
    Motion m_Motion = Motion::FORWARD;
...
int main(int argc, char** argv) {
    ...
    NavigationManager explorer(parameters, nodeHandle);

    ros::Rate r(60);
    const Pose2D& currentPosition = explorer.getCurrentPosition();
    while (ros::ok()) {
        ROS_INFO("Current Motion %d", explorer.getMotion());
        ...
        switch (explorer.getMotion()) {
            case NavigationManager::Motion::FORWARD:
            {
                if (explorer.getDistanceToTarget() < 0.1) {
                    explorer.setMotion(NavigationManager::Motion::FINISH);
                }
                if (fabs(angleToTarget) > ANGLE_EPSILON) {
                    ...
                    if (explorer.getFrontRange() > DISTANCE_EPSILON) {
                        ...
                        explorer.setMotion(NavigationManager::Motion::LEFT);
                    } else if (explorer.getRightRange() > DISTANCE_EPSILON) {
                        explorer.setMotion(NavigationManager::Motion::RIGHT);
                    } else {
                        explorer.setMotion(NavigationManager::Motion::BACK);
                    }
                }
                break;
            }

            case NavigationManager::Motion::LEFT:
            {
                if (fabs(-M_PI_2 - angleToTarget) > ANGLE_EPSILON) {
                    ...
                    if (explorer.getFrame() < FRAME_LIMIT) {
                        ...
                        if (explorer.getRightRange() > DISTANCE_EPSILON) {
                            explorer.setMotion(NavigationManager::Motion::FORWARD);
            ...
            case NavigationManager::Motion::RIGHT:
            {
                if (fabs(M_PI_2 - angleToTarget) > ANGLE_EPSILON) {
                    if ((M_PI_2 - angleToTarget) < 0.0) {
                        explorer.rotate(ANGULAR_SPEED);
            ...
            case NavigationManager::Motion::BACK:
            {
            ...
            case NavigationManager::Motion::FINISH:
