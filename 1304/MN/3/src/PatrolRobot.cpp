...
using namespace std;
static string LOST_ROBOT = "LOST";
static string PATROL_ROBOT = "PATROL";
...
private:
	...
	void move(const std::string& targetFrame, const std::string& sourceFrame) {
		...
		while (nh.ok()) {
			...
			cout << "Patrol fabs(" << fabs(transform.getOrigin().x()) << ", " << fabs(transform.getOrigin().y()) << ")" << endl;		
	...
	void broadcastPosition() {
		...
		cout << "Patrol robot (" << name <<") went to x:" << x << " y:" << y << endl;
...
public:
	PatrolRobot(ros::NodeHandle &nh, string name, string nameWalker) : nh(nh), name(name), nameWalker(nameWalker) {
		id = 2;
		x = y = z = 0;
	}
...
int main(int argc, char **argv) {
	ros::init(argc, argv, "patrol_robot");


	ros::NodeHandle nh;
	PatrolRobot patrol(nh, PATROL_ROBOT, LOST_ROBOT);
	patrol.initVisualisation();
	patrol.search();
	patrol.comeback();
