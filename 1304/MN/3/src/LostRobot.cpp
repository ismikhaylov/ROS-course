...
private:
	...
	void follow() {
		std::cout << "The lost robot start to go" << std::endl;
	...
	void broadcastPosition() {
		...
		cout << "Lost robot (" << name <<") went to x:" << x << " y:" << y << endl;
...
public:
	LostRobor(ros::NodeHandle &nh, string name) : nh(nh), name(name) {
		isFollow = false;
		id = 1;
		x = y = z = 0;
	}
	...
	ros::Subscriber listen() {
		return nh.subscribe("/voice", 100, &LostRobor::handlerVoice, this);
	}
