...
void moveFinder(tf::TransformListener& listener, ros::Publisher& finder) {
    ...
    if ((std::abs(transform.getOrigin().x()) < CONNECT_RANGE) && (std::abs(transform.getOrigin().y()) < CONNECT_RANGE)) {
    	found = true;
    	logmsg << " FOUND! ";
    }
    ...
    if (transform.getOrigin().x() < 0) {
    	vel_msg.x = (std::abs(transform.getOrigin().x()) < STEP)? transform.getOrigin().x() : STEP;
    } else {
    	vel_msg.x = (transform.getOrigin().x() < STEP)? transform.getOrigin().x() : (-1) * STEP;
    }

    if (transform.getOrigin().y() < 0) {
    	vel_msg.y = (std::abs(transform.getOrigin().y()) < STEP)? transform.getOrigin().y() : STEP;
    } else {
